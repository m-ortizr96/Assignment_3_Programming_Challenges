...
genes.each do |gene|
  ...
  seq_obj = seq(gene) # we obtain the sequence of each gene


  unless seq_obj.nil?
    hash_features = match_exon(seq_obj) # we obtain the features inside exons of the gene
    ...
    if hash_features.empty?
      ...
      add_features_embl(gene, hash_features, seq_obj) #we add the new features

      chr = chromosome_features(gene, seq_obj) #we obtain the positions and features from chromosomes

      chromosome_transform(gene, hash_features, chr) #we adapt the chromosome positiosn
